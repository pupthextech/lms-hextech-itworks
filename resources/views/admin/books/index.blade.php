                    ...
                    <tr class="text-center">
                        ...
                        <td>
                            <span @class([
                                ...
                                'badge-danger' => $book->status == 'inactive',
                            ])>{{ $book->status}}</span>
                        </td>
                        <td>
                            <form action="{{ route('books.destroy',$book->id) }}" method="POST">
                                <a class="btn btn-sm btn-success mr-2" href="{{ route('books.edit',$book->id) }}" role="button">Edit</a>
...
<script>
    $('.del').on('click', function() {
        Swal.fire({
            title: 'Delete book? You can\'t reverse this!',
            icon: 'question',
            showDenyButton: true,
            ...
            buttons: [
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        window.location = '{{ URL::to('admin/books/create') }}';
                    },
                }
            ],
        }
