                ...
                @foreach($categoryList as $category)
                    <tr class="text-center">
                        ...
                        <td>
                            <span @class([
                                'badge',
                                'badge-success' => $category->category_status == 'Enable',
                                'badge-danger' => $category->category_status == 'Disabled',
                            ])>{{ $category->category_status}}</span>
                        </td>
                        <td>{{ $category->created_at }}</td>
                        <td>
                            <form action="{{ route('categories.destroy',$category->id) }}" method="POST">
                                <a class="btn btn-sm btn-success mr-2" href="{{ route('categories.edit',$category->id) }}" role="button">Edit</a>
...
<script>
    $('.del').on('click', function() {
        Swal.fire({
            title: 'Delete category? You can\'t reverse this!',
            icon: 'question',
            showDenyButton: true,
            ...
            buttons: [
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        window.location = '{{ URL::to('admin/categories/create') }}';
                    },
                }
            ],
        }
